...
class Output extends Utility {
  ...
  public function loadPlugins() {
    ...
    // List of already required plugins.
    $required_plugins = array();

    for ($i = 0; $i < $recursion_depth; ++$i) {

      // Get plugins directories.
      $directories = glob("content/plugins/*", GLOB_ONLYDIR);

      foreach ($directories as $directory) {

        // Get directory name without path.
        $directory_name = str_replace("content/plugins/", "", $directory);
        ...
        if (file_exists("{$directory}/{$directory_name}.php")) {
          ...
          if (!in_array($directory_name, $required_plugins)) {

            // Require extension source file.
            require "{$directory}/{$directory_name}.php";

            // Add plugin to the list, so it won't be required again.
            array_push($required_plugins, $directory_name);
          }
          ...
          if (class_exists($class_name)) {
            ...
            if (method_exists($class_name, "setDatabaseHandle")) {
              ...
              // Pass the database handle over to the plugin.
              $Plugin->setDatabaseHandle($this->DatabaseHandle);
            }
          }
        }
      }
